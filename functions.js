...
export const createNewStudent = async () => {
    ...
    if (studentStatus) {
        ...
        studentId++;
        const studentCreated = { studentId, firstName, lastName, course, balance, fees };
...
export const showStatus = async () => {
    ...
    if (studentStatus) {
        const studentIndex = students.findIndex(fName => fName.firstName === studentName.firstName);
        let { firstName, lastName, course, balance, fees, studentId } = students[studentIndex];
        var Course = "dummy";
        if (course === "Artificial Intelligence: 5000rs") {
            Course = "Artificial Intelligence";
        }
        else if (course === "Metaverse: 4000rs") {
            Course = "Metaverse";
        }
        else if (course === "Blockchain Technology: 8000rs") {
            Course = "Blockchain Technology";
        }
        else if (course === "Digital Marketing: 3000rs") {
            Course = "Digital Marketing";
        }
        else if (course === "Content Marketing: 3000rs") {
...
export const isFeePaid = async () => {
    ...
    if (studentStatus) {
        const studentIndex = students.findIndex(fName => fName.firstName === answer.name && fName.lastName === answer.lname);
        if (answer.fees) {
...
export const depositBalance = async () => {
    ...
    if (studentStatus) {
        const studentIndex = students.findIndex(fName => fName.firstName === answer.name);
...
export const RemoveStudent = async () => {
    ...
    const studentIndex = students.findIndex(fName => fName.firstName === removeStudent.student && fName.lastName === removeStudent.student1);
    if (studentIndex != -1) {
